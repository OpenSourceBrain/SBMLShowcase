...
def temp_sedml_file_if_not_empty(sedml_filepath, temp_sedml_filepath):
    ...
    if temp_sedml_filepath:
        if os.path.exists(temp_sedml_filepath):
            with open(temp_sedml_filepath) as file:
                sedstr = file.read()

    if sedstr == "":
        with open(sedml_filepath) as file:
            sedstr = file.read()
...
def add_xmlns_sbml_attribute(sedml_filepath, sbml_filepath, temp_sedml_filepath=None):
    ...
    with open(sbml_filepath) as file:
        sbml_str = file.read()
...
def xmlns_fbc_attribute_missing(sbml_filepath, sedml_filepath):
    ...
    """
    with open(sbml_filepath, encoding="utf-8") as file:
        sbmlstr = file.read()

    with open(sedml_filepath, encoding="utf-8") as file:
        sedstr = file.read()
...
def move_d1_files(file_paths, plot_dir="d1_plots"):
    for fpath in file_paths:
        ...
        engine = next((e for e in ENGINES if e in fpath), "unknown")
...
def d1_plots_dict(d1_plots_path="d1_plots"):
    ...
    d1_plots_dict = {
        e: d1_plot for e in ENGINES for d1_plot in d1_plots if e in d1_plot
    }
...
def create_results_table(results, sbml_filepath, sedml_filepath, output_dir):
    ...
    sbml_incompatible_ENGINES = [
        e for e in ENGINES if "sbml" not in ENGINES[e]["formats"][0]
    ]
