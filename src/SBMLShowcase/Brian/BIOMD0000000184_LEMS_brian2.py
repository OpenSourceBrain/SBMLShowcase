...
if show_gui:
    ...
    trace_disp1__X__S = StateMonitor(
        OneComponentPop, "X", record=[0]
    )  # X__S (Type: Line:  scale=1.0 (dimensionless) timeScale=1.0 (dimensionless))
    trace_disp1__Y__S = StateMonitor(
        OneComponentPop, "Y", record=[0]
    )  # Y__S (Type: Line:  scale=1.0 (dimensionless) timeScale=1.0 (dimensionless))
    trace_disp1__Z__S = StateMonitor(
        OneComponentPop, "Z", record=[0]
    )  # Z__S (Type: Line:  scale=1.0 (dimensionless) timeScale=1.0 (dimensionless))

# Saving to file: Lavrentovich2008_Ca_Oscillations.dat, reference: outputFile1
record_outputFile1__os_X = StateMonitor(
    OneComponentPop, "X", record=[0]
)  # os_X (Type: OutputColumn)
record_outputFile1__os_Y = StateMonitor(
    OneComponentPop, "Y", record=[0]
)  # os_Y (Type: OutputColumn)
record_outputFile1__os_Z = StateMonitor(
    OneComponentPop, "Z", record=[0]
)  # os_Z (Type: OutputColumn)

print(
    "Running simulation for %s (dt = %s, #steps = %s, code generation target = %s)"
    % (duration, defaultclock.dt, steps, prefs.codegen.target)
)
...
# Saving to file: Lavrentovich2008_Ca_Oscillations.dat, reference: outputFile1
all_outputFile1 = np.array(
    [
        record_outputFile1__os_X.t,
        record_outputFile1__os_X.X[0],
        record_outputFile1__os_Y.Y[0],
        record_outputFile1__os_Z.Z[0],
    ]
)
...
if show_gui:
    ...
    # Display: Component(id=disp1 type=Display)
    display_disp1 = plt.figure(
        "Simulation of SBML model: Lavrentovich2008_Ca_Oscillations from file: BIOMD0000000184.xml"
    )
    plot_X__S = display_disp1.add_subplot(111, autoscale_on=True)
    plot_X__S.plot(
        trace_disp1__X__S.t / second,
        trace_disp1__X__S.X[0],
        color="#000000",
        label="X__S",
    )
    plot_X__S.legend()
    ...
    plot_Y__S.plot(
        trace_disp1__Y__S.t / second,
        trace_disp1__Y__S.Y[0],
        color="#ff0000",
        label="Y__S",
    )
    plot_Y__S.legend()
    ...
    plot_Z__S.plot(
        trace_disp1__Z__S.t / second,
        trace_disp1__Z__S.Z[0],
        color="#00ff00",
        label="Z__S",
    )
    plot_Z__S.legend()
