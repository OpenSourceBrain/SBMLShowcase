...
def create_test_suite_results_tables():
    ...
    subfolders = [f for f in os.listdir(starting_dir) if os.path.isdir(f) and f.startswith('BIOMD')]
    ...
    for subfolder in subfolders:
        ...
        # get sbml filename ends with .xml
        sbml_file_name = [f for f in os.listdir(subfolder_path) if f.endswith('.xml')][0]
