        ...
        if subfolder in remote_results.keys():
            for e in list(remote_results[subfolder].keys()):
                ...
                mtab[mtab_remote_outcome_key] = (
                    f'<details><summary>{remote_results[subfolder][e]["status"]}</summary>[Download]({remote_links[subfolder][e]["download"]})<br>[Logs]({remote_links[subfolder][e]["logs"]})<br>[View]({remote_links[subfolder][e]["view"]})<br><br>HTTP response: {str(remote_links[subfolder][e]["response"])}</details>'
                )
